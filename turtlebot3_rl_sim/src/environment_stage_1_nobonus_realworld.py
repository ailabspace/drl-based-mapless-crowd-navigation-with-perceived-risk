...
class Env:
    def __init__(self, action_dim=2, max_step=200):
        ...
        self.bounding_box_size = 0.0210  # 0.0105090183944
    ...
    def get_distance_from_point(self, pstart, p_end):
        a = np.array((pstart.x + self.starting_point.x, pstart.y + self.starting_point.y, pstart.z))
    ...
    def get_state(self, scan, step_counter=0, action=[0, 0]):
        ...
        for i in range(len(obstacle_poses)):
            if current_scans_is_gt[i] is True:
                ...
                filtered_scan_ranges.append(scan_range[i])
            ...
            # Is the first object a wall type or an obstacle type?
                ...
                if du_count != 1:
                    if change_grads[i + 1] == 0:
                        ...
                        last_type = _scans_object_type[i]
                        du_count = 0
                    elif abs(change_grads[i] - change_grads[i + 1]) == 0:
                        _scans_object_type[i] = ["w", round(filtered_scan_ranges[i], 3),
                                                 filtered_front_obstacle_poses[i]]
                        last_type = _scans_object_type[i]
                        du_count = 0
        ...
        for i in range(len(scans_object_type)):
            _closest_scans, _obj_types, _obj_poses = [], [], []
            for j in range(len(scans_object_type[i])):
                estimated_scans_object_types.append(scans_object_type[i][j][0])
                estimated_scans_distances.append(scans_object_type[i][j][1])
                estimated_scans_object_poses.append(scans_object_type[i][j][2])
        ...
        for i in range(len(segmented_scan_object_types)):
            _center_item = len(segmented_scan_object_types[i]) / 2
            estimated_obstacle_count = utils.estimate_num_obs_scans(segmented_scan_object_distances[i][_center_item],
                                                                    self.max_scan_range,
                                                                    self.min_scan_range)
            current_obstacle_count = segmented_scan_object_types[i].count('o')
            confirmed_obstacle_score = float(current_obstacle_count) / min(len(segmented_scan_object_types[i]),
                                                                           estimated_obstacle_count)

            if len(set(segmented_scan_object_types[i])) > 1:  # Not all identical
                if confirmed_obstacle_score >= 0.5:
                    if segmented_scan_object_types[i].count('o') > segmented_scan_object_types[i].count('w'):
                        confirmed_scan_object.append(['o', segmented_scan_object_poses[i][_center_item],
                                                      segmented_scan_object_distances[i][_center_item]])
                    else:
                        confirmed_scan_object.append(['w', segmented_scan_object_poses[i][_center_item],
                                                      segmented_scan_object_distances[i][_center_item]])
                else:
                    if len(segmented_scan_object_types[i]) <= estimated_obstacle_count:
                        if segmented_scan_object_types[i].count('o') > segmented_scan_object_types[i].count('w'):
                            confirmed_scan_object.append(['o', segmented_scan_object_poses[i][_center_item],
                                                          segmented_scan_object_distances[i][_center_item]])
                        else:
                ...
                if 'w' in segmented_scan_object_types[i]:
                    if len(segmented_scan_object_types[i]) <= min(len(segmented_scan_object_types),
                                                                  estimated_obstacle_count):
                        pass
                    else:
                        confirmed_scan_object.append(['w', segmented_scan_object_poses[i][_center_item],
        ...
        # print("KEYS AFTER: ", self.tracked_obstacles_keys)
        iou_copy = iou[:]
            ...
            if iou_copy[i]:
                ...
                if max(iou_copy[i]) > 0.0:
                    ...
                    if len(self.tracked_obstacles) > i:
                        # print("REMOVED TRACKING BECAUSE ASSOCIATION WAS LOST")
        ...
        if len(self.agent_pose_deque) == 2:
            ...
            for i in range(len(tracked_obstacles_key_copy)):
                ...
                if distance_to_collision_point is not None:
                    if resultant_vel == 0:
                        time_to_collision = 0
                        _collision_prob = 1.0 * (utils.compute_general_collision_prob(
        ...
        if not self.done:
            if min(scan_range) < self.min_scan_range:
                print("DONE: MINIMUM RANGE")
                print("MINIMUM: ", str(min(scan_range)))
                self.done = True
        ...
        agent_orientation = [round(self.yaw, 3)]  # changed from self.yaw (fixed value)
        ...
        general_obs_distance = scan_range
