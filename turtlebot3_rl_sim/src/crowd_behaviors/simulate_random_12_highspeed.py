...
class Moving():
    ...
    def moving_1(self):
        ...
        random_vel_x = [random.uniform(-0.5, 0.5) for i in range(14)]  # Default: +/- 0.254
        random_vel_y = [random.uniform(-0.5, 0.5) for i in range(14)]  # Default: +/- 0.254

        while True:
            current_time = time.time()
            elapsed_time = current_time - start_time
            # print(elapsed_time)
            if elapsed_time > 0.1:  # Default: 2.25, 0.5
                break
            else:
