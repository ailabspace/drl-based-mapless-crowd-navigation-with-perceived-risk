...
class Moving():
    ...
    def moving_1(self):
        ...
        random_vel_x = [random.uniform(-0.04, 0.04) for i in range(14)]  # Default: +/- 0.254
        random_vel_y = [random.uniform(-0.04, 0.04) for i in range(14)]  # Default: +/- 0.254
        speed = 0.04
        ...
        while True:
            ...
            if elapsed_time > 0.5:  # Default: 2.25, 0.5
                ...
                self.move_model('obstacle_1', self.model_1_pose, 0.0, speed, 0)
                self.move_model('obstacle_2', self.model_2_pose, -speed, speed, 0)
                self.move_model('obstacle_3', self.model_3_pose, -speed, speed, 0)
                self.move_model('obstacle_4', self.model_4_pose, 0.0, speed, 0)
                self.move_model('obstacle_5', self.model_5_pose, -speed, 0.0, 0)
                self.move_model('obstacle_6', self.model_6_pose, -speed, 0.0, 0)
                self.move_model('obstacle_7', self.model_7_pose, 0.0, -speed, 0)
                self.move_model('obstacle_8', self.model_8_pose, -speed, 0.0, 0)
                self.move_model('obstacle_9', self.model_9_pose, 0.0, -speed, 0)
                self.move_model('obstacle_10', self.model_10_pose, -speed, -speed, 0)
                self.move_model('obstacle_11', self.model_11_pose, -speed, -speed, 0)
                self.move_model('obstacle_12', self.model_12_pose, -speed, -speed, 0)
                self.move_model('obstacle_13', self.model_13_pose, -speed, speed, 0)
                self.move_model('obstacle_14', self.model_14_pose, -speed, 0.0, 0)
                self.move_model('obstacle_15', self.model_15_pose, -speed, 0.0, 0)
                self.move_model('obstacle_16', self.model_16_pose, speed, speed, 0)
                self.move_model('obstacle_17', self.model_17_pose, speed, speed, 0)
                self.move_model('obstacle_18', self.model_18_pose, speed, -speed, 0)
                self.move_model('obstacle_19', self.model_19_pose, speed, -speed, 0)
                self.move_model('obstacle_20', self.model_20_pose, -speed, speed, 0)
    ...
    def move_model(self, model_name, pose, vel_x, vel_y, vel_z):
        ...
        # if abs(pose.position.x - self.position.x) < speed and abs(pose.position.y - self.position.y) < speed:
