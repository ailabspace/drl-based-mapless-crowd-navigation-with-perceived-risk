...
class Moving():
    ...
    def moving_1(self):
        ...
        random_vel_x = [random.uniform(-0.1, 0.1) for i in range(14)]  # Default: +/- 0.254
        random_vel_y = [random.uniform(-0.1, 0.1) for i in range(14)]  # Default: +/- 0.254

        while True:
            current_time = time.time()
            elapsed_time = current_time - start_time
            # print(elapsed_time)
            if elapsed_time > 2.25:  # Default: 2.25, 0.5
                break
            else:
                self.move_model('obstacle_1', self.model_1_pose, random_vel_x[0], random_vel_y[0], 0)
                self.move_model('obstacle_3', self.model_3_pose, random_vel_x[1], random_vel_y[1], 0)
                self.move_model('obstacle_4', self.model_4_pose, random_vel_x[2], random_vel_y[2], 0)
                self.move_model('obstacle_5', self.model_5_pose, random_vel_x[3], random_vel_y[3], 0)
                self.move_model('obstacle_7', self.model_7_pose, random_vel_x[4], random_vel_y[4], 0)
                self.move_model('obstacle_9', self.model_9_pose, random_vel_x[5], random_vel_y[5], 0)
                self.move_model('obstacle_11', self.model_11_pose, random_vel_x[6], random_vel_y[6], 0)
                self.move_model('obstacle_12', self.model_12_pose, random_vel_x[7], random_vel_y[7], 0)
