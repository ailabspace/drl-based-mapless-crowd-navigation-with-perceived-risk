...
if __name__ == '__main__':
    rospy.init_node('sarsa_training', anonymous=True, log_level=rospy.WARN)
    ...
    utils.remove_logfile_if_exist(result_outdir, "sarsa_training")
    ...
    sarsa = sarsa.Sarsa(actions=range(3), alpha=Alpha, gamma=Gamma, epsilon=Epsilon)
        ...
        for step in range(nsteps):
            ...
            if done:
                if (ep + 1) % 100 == 0:
                    ...
                    sarsa.save_q(qtable, model_outdir, "sarsa_qtable_ep" + str(ep+1))
                rospy.logwarn("DONE")
                data = [ep + 1, success_episode, failure_episode, cumulated_reward, step + 1]
                utils.record_data(data, result_outdir, "sarsa_training_dis_test")
